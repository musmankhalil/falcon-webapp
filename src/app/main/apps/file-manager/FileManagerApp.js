...
import { selectFileById, getFiles, postFile } from './store/filesSlice';
...
function FileManagerApp() {
	...
	const postData = (data) => {
		dispatch(postFile({
			"id": Math.random(),
			"name": data.name.substring(0, data.name.lastIndexOf(".") + 1).slice(0, -1),
			"type": data.name.split('.')[data.name.split('.').length -1],
			"owner": user.data.displayName,
			"size": data.size + " Kb",
			"modified": moment(data.lastModified).format('MMM D, YYYY'),
			"opened": moment().format('MMM D, YYYY'),
			"created": moment().format('MMM D, YYYY'),
			"extention": "",
			"location": "APK File Manager",
			"offline": false,
